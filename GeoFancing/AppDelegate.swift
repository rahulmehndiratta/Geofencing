...
extension AppDelegate: CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didEnterRegion region: CLRegion) {
        if region is CLCircularRegion {
            ...
            {
                let entryExit  = (TblEntryExit.findOrCreate(dictionary: ["regionIdentifier":region.identifier]) as? TblEntryExit)!
                entryExit.entryLat = manager.location?.coordinate.latitude ?? 0.0
                entryExit.entryLong = manager.location?.coordinate.longitude ?? 0.0
                entryExit.entryTime = "\(Date.currentTimeStamp)"
                entryExit.regionIdentifier = region.identifier
                entryExit.regionName = geoFance.title
    ...
    func locationManager(_ manager: CLLocationManager, didExitRegion region: CLRegion) {
        if region is CLCircularRegion {
            ...
            {
                let entryExit  = (TblEntryExit.findOrCreate(dictionary: ["regionIdentifier":region.identifier]) as? TblEntryExit)!
                entryExit.exitLat = manager.location?.coordinate.latitude ?? 0.0
                entryExit.exitLong = manager.location?.coordinate.longitude ?? 0.0
                entryExit.exitTime = "\(Date.currentTimeStamp)"
                entryExit.regionIdentifier = region.identifier
                CoreData.sharedInstance.saveContext()
